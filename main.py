...
class Main:
    ...
    def set_emulator(self):
        ...
        if not selectedItem or not selectedItem.parent():
            QMessageBox.warning(
                inst.ui, "Selection Error", "Please select an emulator."
            )
            return False
        ...
        if self.emulator != emulatorName:
            ...
            selectedEmulator = next(
                (em for em in emulatorData.values() if em["name"] == self.emulator),
                None,
            )
            if selectedEmulator:
                ...
                def set_releases_url(owner, repo):
                    if selectedEmulator["name"] in nonGithubHosts:
                        hostInfo = nonGithubHosts[selectedEmulator["name"]]
                        releasesUrl = hostInfo["api_url"].format(owner=owner, repo=repo)
                    else:
        ...
        return (
            True  # To avoid the function from switching index even on non-select item
        )
    ...
    def Install_path(self):
        ...
        selectedDirectory = QFileDialog.getExistingDirectory(
            inst.install, "Select Installation Directory", currentPath
        )
    ...
    def add_releases_to_combobox(self):
        ...
        try:
            ...
            QMessageBox.critical(
                inst.ui, "Error", f"Failed to fetch releases: {str(e)}"
            )
    ...
    def emulator_updater(self):
        ...
        if self.emulator in [
            ...
            "NooDS",
            "Duckstation",
        ]:  # TODOL Add this type to json database
            reply = QMessageBox.question(
                inst.ui,
                "Rolling-Release Emulator Detected",
                f"{self.emulator} uses rolling-releases instead of numbered releases. This means that the latest version may not be the one you have installed (We cannot detect the version). Would you like to proceed with the download anyway?",
            ...
            if reply == QMessageBox.StandardButton.Yes:
                try:
                    exePath = os.path.join(
                        reg[0], inst.online.emulatorDatabase[self.emulator]["exe_path"]
                    )
                    subprocess.Popen([exePath])
                except Exception as e:
                    QMessageBox.critical(
                        inst.ui, "Error", f"Failed to open emulator: {e}"
                    )
                return
            else:
                pass
        ...
        if latestTag > installedEmulator:
            reply = QMessageBox.question(
                inst.ui,
                "Update Found",
                "Would you like to update " + self.emulator + " to " + latestTag + "?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            )
            if reply == QMessageBox.StandardButton.Yes:
                self.installMode = "Update"
            ...
            QMessageBox.information(
                inst.ui,
                "No Update Found",
                f"You are up to date with the latest version of {self.emulator}",
    ...
    def prepare_Download(self):
        ...
        if not self.windowsAssets:
            QMessageBox.critical(
                inst.ui,
                "Error",
                f"No suitable Windows download found for {self.emulator}. Please try another release.",
            )
        ...
        if len(self.windowsAssets) > 1:
            options = "\n".join(
                [
                    f"{idx + 1}: {asset['name']}"
                    for idx, asset in enumerate(self.windowsAssets)
                ]
            )
            ...
            if ok:
                selectedAsset = next(
                    asset for asset in self.windowsAssets if asset["name"] == choice
                )
            else:
                QMessageBox.critical(
                    inst.ui, "Error", "Please select an asset to download"
                )
                inst.ui.qt_index_switcher(3)
                return
        else:
    ...
    def extract_and_install(self, tempFile, extractTo):
        ...
        try:
            ...
            try:
                    ...
                    for file in files:
                        if file.endswith((".zip", ".7z")):
                            ...
                            try:
                                if file.endswith(".7z"):
                                    result = subprocess.run(
                                        [sevenZip, "x", nestedPath, f"-o{root}", "-y"],
                                        capture_output=True,
                                        text=True,
                                    )
                                    if result.returncode != 0:
                                        raise Exception(
                                            f"7zip extraction failed: {result.stderr}"
                                        )
                                else:
                ...
                QMessageBox.critical(
                    inst.ui, "Extraction Error", f"Failed to extract files: {str(e)}"
                )
                return
            finally:
            ...
            QMessageBox.critical(
                inst.ui, "File Error", f"Failed to process downloaded file: {str(e)}"
            )
            return
            ...
            try:
                ...
                for idx, item in enumerate(items):
                    try:
                        ...
                        QMessageBox.warning(
                            inst.ui, "Copy Warning", f"Failed to copy {item}: {str(e)}"
                        )
                        continue
            except Exception as e:
                QMessageBox.critical(
                    inst.ui, "Installation Error", f"Failed to install files: {str(e)}"
                )
                return
            finally:
                shutil.rmtree(self.tempExtractFolder, ignore_errors=True)
    ...
    def installation_complete(self):
        ...
        if not exeName:
            QMessageBox.critical(
                inst.ui,
                "Error",
                f"Executable path not found for emulator: {self.emulator}",
            )
            return
        ...
        if not os.path.exists(testPath):
            try:
                ...
                QMessageBox.critical(
                    inst.ui,
                    "Error",
                    "You do not have the necessary permissions to uninstall from this directory. Please run the application as an administrator.",
                )
                return True
    ...
    def check_reg(self):
        try:
            self.registryKey = winreg.OpenKey(
                winreg.HKEY_CURRENT_USER,
                f"Software\\{self.emulator}",
                0,
                winreg.KEY_READ,
            )
            self.instValue = winreg.QueryValueEx(self.registryKey, "Install_Dir")
            self.updateValue = winreg.QueryValueEx(self.registryKey, "Version")
            self.assetVersion = winreg.QueryValueEx(self.registryKey, "Asset_version")
    ...
    def create_reg(self):
        ...
        self.registryKey = winreg.OpenKey(
            winreg.HKEY_CURRENT_USER, f"Software\\{self.emulator}", 0, winreg.KEY_WRITE
        )
        if self.installMode == "Install":
            winreg.SetValueEx(
                self.registryKey,
                ...
                0,
                winreg.REG_SZ,
                inst.install.installationPathLineEdit.text(),
            )
            winreg.SetValueEx(
                self.registryKey,
                "Asset_version",
    ...
    def define_shortcut(self, target, locationType):
        ...
        if not os.path.exists(target):
            QMessageBox.critical(
                inst.ui, "Error", f"Shortcut target does not exist: {target}"
            )
            return
    ...
    def uninstall(self):
        ...
        if uninstallReg[0] is not None:
            desktopShortcut = os.path.join(
                os.environ["USERPROFILE"], "Desktop", f"{self.emulator}.lnk"
            )
            startMenuShortcut = os.path.join(
                os.environ["APPDATA"],
                "Microsoft",
                "Windows",
                "Start Menu",
            ...
            reply = QMessageBox.question(
                inst.ui,
                "Uninstall",
                f"Are you sure you want to uninstall {self.emulator} located on {self.instValue}?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            )
            if reply == QMessageBox.StandardButton.No:
                ...
                if dirpath.exists() and dirpath.is_dir():
                    ...
                    winreg.DeleteKey(
                        winreg.HKEY_CURRENT_USER, f"Software\\{self.emulator}"
                    )
                    QMessageBox.information(
                        inst.ui,
                        "Uninstall",
                        f"{self.emulator} has been successfully uninstalled.",
                    )
                    self.emulator = None
                    inst.ui.qt_index_switcher(1)
                else:
                    QMessageBox.critical(
                        inst.ui,
                        "Error",
                        "The directory might have been moved or deleted. Please reinstall the program.",
                    )
                    winreg.DeleteKey(
                        winreg.HKEY_CURRENT_USER, f"Software\\{self.emulator}"
                    )
            ...
            QMessageBox.critical(
                inst.ui,
                "Error",
                ("Failed to read the registry key. Try and reinstall again!"),
            )
            inst.ui.qt_index_switcher(1)
